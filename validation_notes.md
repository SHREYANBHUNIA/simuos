...
The New run control now creates a fresh generated workload, resets scheduling parameters to the baseline Round Robin configuration, returns the user to the scheduler laboratory, and updates an accessible readiness label. Its interaction test confirms that clicking the header control advances the run state to `Run #2 ready` and replaces the editable workload values. The control was also reviewed in the desktop interface.
